...
public class KerberosServiceRequestToken
     ...
     * Jannik Upmann: switching to Java Util Base64 Encoder 
     */
    public String getEncodedResponseToken() throws UnsupportedEncodingException {
        if (!hasResponseToken()) {
            throw new IllegalStateException("Unauthenticated or no response token");
        }

        return new String(Base64.getEncoder().encode(ticketValidation.responseToken()), "UTF-8");
    }
     ...
     * 
     * Jannik Upmann: Switching to Lmbda 
     */
    public byte[] decrypt(final byte[] data, final int offset, final int length) throws PrivilegedActionException {
        return Subject.doAs(getTicketValidation().subject(), (PrivilegedExceptionAction<byte[]>) () -> {
